...
const FinancialHealthForm = () => {
    ...
    const calculateFinancialHealth = () => {
        ...
        const monthlyIncome = Number(answers[1]) + Number(answers[2] || 0);
        const essentialExpenses = Number(answers[3]);
        const lifestyleExpenses = Number(answers[4]);
        const totalSavings = Number(answers[5]);
        const totalInvestments = Number(answers[6] || 0);
        const monthlyInstallments = Number(answers[7] || 0);
        const dependents = Number(answers[8]);
        const monthlySavings = Number(answers[9]);

        // Calculate total monthly expenses
        const totalMonthlyExpenses = essentialExpenses + lifestyleExpenses + monthlyInstallments;

        // Calculate emergency fund status
        const targetEmergencyFund = totalMonthlyExpenses * 6;
        const currentEmergencyFund = totalSavings;
        const emergencyFundRatio = currentEmergencyFund / targetEmergencyFund;

        // Calculate debt ratio
        const debtRatio = (monthlyInstallments / monthlyIncome) * 100;

        // Calculate savings ratio
        const savingsRatio = (monthlySavings / monthlyIncome) * 100;

        // Determine statuses
        const emergencyFundStatus = emergencyFundRatio >= 1 ? "Cukup" : emergencyFundRatio >= 0.5 ? "Sedang" : "Kurang";
        const debtStatus = debtRatio <= 30 ? "Aman" : debtRatio <= 50 ? "Waspada" : "Bahaya";
        const savingsStatus = savingsRatio >= 20 ? "Bagus" : savingsRatio >= 10 ? "Cukup" : "Perlu Ditingkatkan";

        // Generate recommendations
        const recommendations = [];

        if (emergencyFundStatus !== "Cukup") {
            recommendations.push(`Prioritaskan membangun dana darurat hingga Rp${formatNumber(targetEmergencyFund)}`);
        }

        if (debtStatus !== "Aman") {
            recommendations.push("Kurangi beban cicilan, hindari tambahan utang baru");
        }

        if (savingsStatus !== "Bagus") {
            recommendations.push("Tingkatkan porsi menabung minimal 20% dari penghasilan");
        }

        return {
            monthlyIncome,
            totalMonthlyExpenses,
            netMonthlyCashflow: monthlyIncome - totalMonthlyExpenses,
            emergencyFund: {
